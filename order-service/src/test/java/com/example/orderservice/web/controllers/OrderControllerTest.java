...
class OrderControllerTest {
	...
	void setUp() {
		...
		this.orderList.add(Order.builder().id(1L).customerEmail("email1@junit.com").customerAddress("address 1")
				.customerId(1L).build());
		this.orderList.add(Order.builder().id(2L).customerEmail("email2@junit.com").customerAddress("address 2")
				.customerId(2L).build());
		this.orderList.add(Order.builder().id(3L).customerEmail("email3@junit.com").customerAddress("address 3")
				.customerId(3L).build());
	...
	void shouldCreateNewOrder() throws Exception {
		given(orderService.saveOrder(any(OrderDto.class))).willAnswer((invocation) -> invocation.getArgument(0));
	...
	void shouldReturn400WhenCreateNewOrderWithoutText() throws Exception {
		...
		this.mockMvc
				...
				.andExpect(jsonPath("$.title", is("Constraint Violation"))).andExpect(jsonPath("$.status", is(400)))
				.andExpect(jsonPath("$.violations", hasSize(1)))
				.andExpect(jsonPath("$.violations[0].field", is("customerEmail")))
				.andExpect(jsonPath("$.violations[0].message", is("Email can't be blank"))).andReturn();
	}

	@Test
	void shouldUpdateOrder() throws Exception {
		OrderDto orderDto = new OrderDto(1L, "email1@junit.com", "address updated", new ArrayList<>());

		given(orderService.findOrderById(orderDto.getOrderId())).willReturn(Optional.of(orderDto));
		given(orderService.saveOrder(any(OrderDto.class))).willAnswer((invocation) -> invocation.getArgument(0));

		this.mockMvc
				.perform(put("/api/orders/{id}", orderDto.getOrderId()).contentType(MediaType.APPLICATION_JSON)
						.content(objectMapper.writeValueAsString(orderDto)))
				.andExpect(status().isOk()).andExpect(jsonPath("$.customerAddress", is(orderDto.getCustomerAddress())));
	}
	...
	void shouldReturn404WhenUpdatingNonExistingOrder() throws Exception {
		Long orderId = 1L;
		given(orderService.findOrderById(orderId)).willReturn(Optional.empty());
		Order order = Order.builder().id(1L).customerEmail("email1@junit.com").customerAddress("address updated")
				.customerId(1L).build();

		this.mockMvc.perform(put("/api/orders/{id}", orderId).contentType(MediaType.APPLICATION_JSON)
	...
	void shouldDeleteOrder() throws Exception {
		Long orderId = 1L;
		OrderDto order = new OrderDto(null, "email1@junit.com", "address updated", new ArrayList<>());
		given(orderService.findOrderById(orderId)).willReturn(Optional.of(order));
		doNothing().when(orderService).deleteOrderById(orderId);

		this.mockMvc.perform(delete("/api/orders/{id}", orderId)).andExpect(status().isOk())
				.andExpect(jsonPath("$.customerAddress", is(order.getCustomerAddress())));
	}
