...
class ProductControllerTest {
    ...
    void index_shouldRedirectToProductsPage() throws Exception {
        mockMvc.perform(get("/")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/products"));
    }
    ...
    void showProductsPage_shouldRenderProductsWithSpecifiedPage() throws Exception {
        int pageNumber = 2;
        mockMvc.perform(get("/products").param("page", String.valueOf(pageNumber)))
                .andExpect(status().isOk())
                .andExpect(view().name("products"))
    ...
    void products_shouldReturnPagedResultForSpecifiedPage() throws Exception {
        ...
        mockMvc.perform(get("/api/products").param("page", "2"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data", hasSize(3)));
    }
